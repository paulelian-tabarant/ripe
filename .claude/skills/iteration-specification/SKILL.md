...
4. **Write cases as BDD acceptance criteria.** One `Given/When/Then` per behavior or edge
   ...
   unambiguous, not a restatement of the test strategy. Write `Given`, `When`, `Then` each
   on its own line (not inlined into one sentence) — see Output shape below. Per root
   `STANDARDS.md`'s "Behavioral over implementation-detail assertions": before adding a case,
   check whether another case already forces it to hold behaviorally (e.g. a uniqueness
   constraint proven by two requests resolving predictably) — drop it if so, rather than also
   writing a schema/implementation-only case that duplicates what the behavior already proves.
